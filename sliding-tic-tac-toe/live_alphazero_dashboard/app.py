"""
Live, fully manual AlphaZero-style training dashboard - the same UX as
../web_dashboard/app.py (initialize, Start/Pause/Resume/Stop, live math on
screen, adjustable hyperparameters, autosave/resume across restarts), but for
the neural-network + guided-MCTS self-play loop instead of tabular
Q-learning. This is deliberately a SEPARATE, freshly-initialized network from
every checkpoint ../alphazero_selfplay.py's batch runs produced - it never
touches those files, so there's no risk of two processes fighting over the
same checkpoint.
...
from perfect_solver import negamax_value, _enumerate_states, _value_iteration  # noqa: E402
...
_SOLVER_VALUES, _ = _value_iteration(_states, _transitions)
...
def load_session():
    ...
    restored_net = TicTacToeNet(hidden_size=64, seed=42)
...
def _make_fresh_net():
    return TicTacToeNet(hidden_size=64, seed=42)
