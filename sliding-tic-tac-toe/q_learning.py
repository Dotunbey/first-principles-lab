"""
Tabular Q-learning for Sliding Tic-Tac-Toe.

Separate from mcts.py on purpose: this agent needs training before it can
play (self-play, updating a persistent Q-table), whereas mcts.py needs none.
Later, this file's trained agent can be pitted against mcts.py's agent to see
how many training games it takes for Q-learning to "catch up" to MCTS.

Reward scheme is the plain textbook version - reward only at the terminal
move (win=+1, loss=-1, draw=0), nothing shaped in along the way. Every
non-terminal move gets credit only through backward discounting, same
Q(s,a) <- Q(s,a) + alpha*[G - Q(s,a)] update as before.
"""
...
class QLearningAgent:
    ...
    def hash_state(self, game):
        """State = board + phase + whose turn it is. Move-history (needed for
        threefold repetition) is intentionally NOT part of the state - the
        agent judges a position on its own merits, not how it got there."""
...
def evaluate_vs_random(agent, num_games=200):
    ...
    a random opponent (as O). Useful for tracking how strong the agent has
    gotten at a given point in training."""
...
def demonstrate_q_learning():
    print("=== SLIDING TIC-TAC-TOE Q-LEARNING ===")
    print()
